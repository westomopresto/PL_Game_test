...
FText USMGraphK2Node_StateReadNode_TimeInState::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	if (TitleType != ENodeTitleType::MenuTitle)
	{
		...
		if (!StateName.IsEmpty())
		{
			return FText::FromString(FString::Printf(TEXT("Time in State '%s'"), *StateName));
		}
	}
	
	return FText::FromString(TEXT("Time in State"));
}
...
{
	return LOCTEXT("CanEvaluateFromEventTooltipRead", "If the transition is allowed to evaluate from autobinded events.");
}
