	...
	if (NodeTemplate != nullptr)
	{
		...
		if (NodeClass)
		{
			if (USMGraphNode_Base* GraphNode = Cast<USMGraphNode_Base>(NodeTemplate))
			{
				...
				{
					const FString& DefaultNodeName = NodeInstance->GetNodeDisplayName();
					if (!DefaultNodeName.IsEmpty())
					{
						TSharedPtr<INameValidatorInterface> NameValidator = FNameValidatorFactory::MakeValidator(GraphNode);
...
void USMGraphSchema::GetGraphContextActions(FGraphContextMenuBuilder& ContextMenuBuilder) const
{
	...
	if(StateMachineDefault)
	{
		...
		bBaseStateMachinesAllowed = Rules.IsStateAllowed(USMStateMachineInstance::StaticClass());
...
{
	const USMGraphNode_StateNodeBase* StateNodeA = Cast<USMGraphNode_StateNodeBase>(A);
	const USMGraphNode_StateNodeBase* StateNodeB = Cast<USMGraphNode_StateNodeBase>(B);
