...
bool TestHelpers::TryCreateNewNodeAsset(FAutomationTestBase* Test, FAssetHandler& NewAsset, UClass* NodeClass, bool Save)
{
	...
	Test->TestEqual("Construction nodes created", ConstructionNodes.Num(), 2); // Entry and parent call.
	...
	for(UK2Node* Node : ConstructionNodes)
	{
		...
		{
			NodesChecked++;
			Test->TestEqual("Construction entry exists", EntryNode->FunctionReference.GetMemberName(), GET_FUNCTION_NAME_CHECKED(USMNodeInstance, ConstructionScript));
		}
	...
	for (int32 r = 0; r < Rows; ++r)
	{
		...
		for (UEdGraphPin* FromPin : FromPins) 
		{
			...
			for (int32 b = 0; b < Branches; ++b)
			{
				...
				CastChecked<USMStateInstance_Base>(StateNode->GetNodeTemplate())->bStayActiveOnStateChange = bLeaveActive;
				CastChecked<USMStateInstance_Base>(StateNode->GetNodeTemplate())->bAllowParallelReentry = bReEnterStates;
				// Make sure a transition can exist.
				if (FromPin->GetOwningNode()->IsA<USMGraphNode_StateNodeBase>())
				{
					// Make the transition able to be taken.
					...
					Transition->GetNodeTemplateAs<USMTransitionInstance>()->bRunParallel = bRunParallel;
					Transition->GetNodeTemplateAs<USMTransitionInstance>()->bEvalIfNextStateActive = bEvalIfNextStateActive;
				}
...
void TestHelpers::SetNodeClass(FAutomationTestBase* Test, USMGraphNode_Base* Node, TSubclassOf<USMNodeInstance> Class)
{
	...
	Test->TestEqual("Graph properties created", GraphProperties.Num(), ExposedProperties.Num());
}
